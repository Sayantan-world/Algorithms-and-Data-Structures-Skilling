...
int main() {
	...
	int in2[n2];
	int post2[n2];
	for(auto &i: post2) cin >> i;
	for(auto &i: in2) cin >> i;
	Node* root2 = buildTree(in2, post2, n2);
	
	if (isSubtree(root, root2)) cout << 1 << endl;
	else if (isSubtree(root2, root)) cout << 1 << endl;
	else cout << 0 << endl;
