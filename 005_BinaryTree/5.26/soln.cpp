...
bool printAncestors(Node *root, int target, bool& flag) {
	
  if (root == NULL) return false;
  if (root->data == target) return true;
  if ( printAncestors(root->left, target, flag) ||
       printAncestors(root->right, target, flag) ) {
    cout << root->data << " ";
     flag = true;
    return true;
  }
