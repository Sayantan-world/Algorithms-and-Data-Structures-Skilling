...
public:
    ...
    BST* Insert(BST*, int);
    void Inorder(BST*);
    bool inRange(BST*, int, int);
	bool getCountUtil(BST*, int, int, int*);
...
bool BST::inRange(BST* root, int low, int high) {
	
    return root->data >= low && root->data <= high;
}

bool BST::getCountUtil(BST* root, int low, int high, int* count) {
