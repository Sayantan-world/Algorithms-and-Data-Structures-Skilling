...
public:
    ...
    BST* Insert(BST*, int);
    void Inorder(BST*);
    BST* removeOutsideRange(BST*, int, int);
};
...
BST* BST::removeOutsideRange(BST *root, int min, int max) {

   if (root == NULL) return NULL;
   root->left =  removeOutsideRange(root->left, min, max);
   root->right =  removeOutsideRange(root->right, min, max);
   if (root->data < min) {
       BST *rChild = root->right;
       delete root;
       return rChild;
   }
   if (root->data > max) {
       BST *lChild = root->left;
       delete root;
       return lChild;
   }
   return root;
...
int main() {
	...
	if(n > 1){
		...
		root = b.removeOutsideRange(root, x, y);
		b.Inorder(root);
		cout << endl;
	}
	else{
