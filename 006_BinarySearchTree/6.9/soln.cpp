...
int largestBSTUtil(Node* node, int *min_ref, int *max_ref, int *max_size_ref, bool *is_bst_ref) {
    ...
    *max_ref = INT_MIN;
    ...
    if (*is_bst_ref == 1 && node->data > *max_ref) left_flag = true;
    min = *min_ref;
    *min_ref = INT_MAX;
    rs = largestBSTUtil(node->right, min_ref, max_ref, max_size_ref, is_bst_ref);
    if (*is_bst_ref == 1 && node->data < *min_ref) right_flag = true;
    if (min < *min_ref) *min_ref = min;
    if (node->data < *min_ref) *min_ref = node->data;
    if (node->data > *max_ref) *max_ref = node->data;
